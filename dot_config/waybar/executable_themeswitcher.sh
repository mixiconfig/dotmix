...
# -----------------------------------------------------
themes_path="$HOME/.config/waybar/style/"
...
for value in $options; do
	if [ ! $value == "$HOME/.config/waybar/assets" ]; then
		if [ ! $value == "$themes_path" ]; then
			if [ $(find $value -maxdepth 1 -type d | wc -l) = 1 ]; then
				result=$(echo $value | sed "s#$HOME/.config/waybar/style/#/#g")
...
choice=$(echo -e "$listNames" | rofi -dmenu -replace -i -config ~/.config/rofi/styles/style_10.rasi -no-show-icons -width 30 -p "Themes" -format i)
IFS="~"
input=$listNames2
read -ra array <<<"$input"
